...
func TestContainerInstanceTags(t *testing.T) {
	...
	os.Setenv("ECS_CONTAINER_INSTANCE_PROPAGATE_TAGS", "ec2_instance")
	os.Setenv("ECS_CONTAINER_INSTANCE_TAGS", fmt.Sprintf("{\"%s\": \"%s\", \"%s\": \"%s\"}",
		localKey, localValue, commonKey, commonKeyLocalValue))
	agent := RunAgent(t, nil)
	defer func() {
		agent.Cleanup()
		os.Unsetenv("ECS_CONTAINER_INSTANCE_PROPAGATE_TAGS")
		os.Unsetenv("ECS_CONTAINER_INSTANCE_TAGS")
	}()
